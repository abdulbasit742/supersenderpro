...
import {
  PageHeader, Card, KpiCard, Badge, Section,
  SkeletonCard, Skeleton, EmptyState,
} from "@/components/ui-kit";
import {
  Users, DollarSign, ShoppingCart, Handshake, AlertTriangle,
  Clock, TrendingUp, Wifi, Plus, Package, Megaphone, UserPlus, Zap,
  RefreshCw,
} from "lucide-react";
import {
...
import { useBusinessOverview, useProfitSeries, useStockInventory, useOrders } from "@/lib/hooks";
import { pendingTrustData } from "@/lib/mock-reseller";
...
function Dashboard() {
  ...
  const profit    = useProfitSeries(7);
  ...
  const lowStock = stockRows
    .filter((s: StockItem) => Number(s.available ?? 0) <= Number(s.threshold ?? 3))
    .slice(0, 6);
    ...
    <>
      <PageHeader
        ...
        actions={
          <button
            onClick={refetchAll}
            className="inline-flex items-center gap-1.5 px-3 h-9 rounded-md bg-secondary text-sm hover:bg-accent transition-colors"
          >
      ...
      <div className="grid lg:grid-cols-3 gap-4 mb-6">
        <Section title="Weekly Profit Trend">
          {profit.isLoading ? (
            ...
            <div className="h-64">
              <ResponsiveContainer>
                <AreaChart data={profitData?.daily ?? []} margin={{ left: 0, right: 8, top: 8, bottom: 0 }}>
                  <defs>
        ...
        <Section title="Top Tools by Sales">
          {profit.isLoading ? (
            ...
            <div className="h-64">
              <ResponsiveContainer>
                <BarChart data={profitData?.topTools ?? []}>
                  <CartesianGrid stroke="var(--color-border)" strokeDasharray="3 3" />
        ...
        <Section title="Alerts">
          <div className="space-y-2">
            {(summary?.alerts ?? []).slice(0, 5).map((a) => (
              <div key={a.id} className="flex items-start gap-2 p-3 rounded-lg bg-muted">
                <AlertTriangle className={`h-4 w-4 mt-0.5 shrink-0 ${a.severity === "warning" ? "text-warning" : "text-info"}`} />
            ...
            {!overview.isLoading && !summary?.alerts?.length && (
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        <Card className="lg:col-span-2 p-0 overflow-hidden">
          ...
          {orders.isLoading ? (
            ...
            <EmptyState icon={ShoppingCart} title="No orders yet" description="Orders will appear here once customers start buying." />
          ) : (
            <div className="overflow-x-auto">
              <table className="min-w-full text-sm">
                ...
                <tbody>
                  {orderRows.slice(0, 8).map((o: Order) => (
                    <tr key={o.id} className="border-t border-border hover:bg-accent/20 transition-colors">
                      ...
                      <td className="px-4 py-2">
                        <Badge variant={
                          ...
                          o.status === "cancelled"        ? "destructive" : "info"
                        }>
                          {o.status}
                        </Badge>
                      </td>
        ...
        <Section title="Low Stock Alerts">
          {stock.isLoading ? (
            ...
            <EmptyState title="All stock healthy" description="No items below threshold." />
          ) : (
            <div className="space-y-3">
              {lowStock.map((row: StockItem) => {
                  ...
                  <div key={row.id} className={`rounded-lg border p-3 ${qty === 0 ? "border-destructive/40 bg-destructive/10" : "border-warning/40 bg-warning/10"}`}>
                    <div className="flex items-center justify-between mb-2">
                      <div className="font-semibold text-sm">
                        {row.tool} {row.plan} <span className="text-muted-foreground font-normal text-xs">({row.accountType})</span>
                      </div>
                      <Badge variant={qty === 0 ? "destructive" : "warning"}>{qty === 0 ? "Out" : "Low"}</Badge>
                    </div>
                    <div className="h-1.5 rounded-full bg-muted overflow-hidden">
                      <div className={`h-full transition-all ${qty === 0 ? "bg-destructive" : "bg-warning"}`} style={{ width: `${pct}%` }} />
                    </div>
