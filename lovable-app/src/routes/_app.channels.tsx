...
import type { ChannelSource, ChannelItem } from "@/lib/types";

export const Route = createFileRoute("/_app/channels")({
  component: ChannelsPage,
});

interface ChannelItemExt extends ChannelItem {
  content?: string;
  ai_content?: string;
  media_urls?: string[];
  media_type?: string;
  fetched_at?: string;
  published_at?: string;
  channel_sources?: { name: string };
}

interface ChannelSourceExt extends ChannelSource {
  is_active?: boolean;
  social_accounts?: { handle: string };
}

interface Bot { id: string; handle: string; platform: string; }
...
function ChannelsPage() {
  ...
  const { data: sources = [] } = useQuery<ChannelSourceExt[]>({
    queryKey: ["channel-sources"],
    queryFn: () => fnSources() as Promise<ChannelSourceExt[]>,
    enabled: !!user,
    staleTime: 30_000,
  });

  const { data: items = [] } = useQuery<ChannelItemExt[]>({
    queryKey: ["channel-items"],
    queryFn: () => fnItems() as Promise<ChannelItemExt[]>,
    enabled: !!user,
    staleTime: 15_000,
  });

  const { data: bots = [] } = useQuery<Bot[]>({
    queryKey: ["telegram-bots", user?.id],
    queryFn: async () => {
      if (!user) return [];
      const { data } = await supabase.from("social_accounts").select("id, handle, platform").eq("user_id", user.id).eq("platform", "telegram");
      return (data ?? []) as Bot[];
    },
    enabled: !!user,
    staleTime: 120_000,
  ...
  const invalidate = () => qc.invalidateQueries({ queryKey: ["channel-sources"] });
  const invalidateAll = () => { invalidate(); qc.invalidateQueries({ queryKey: ["channel-items"] }); };
  ...
  async function pullAll() {
    ...
    for (const s of sources.filter((x) => x.is_active)) {
      try { const r = await fnPull({ data: { sourceId: s.id } }) as { inserted: number }; total += r.inserted; }
      catch (e: unknown) { toast.error(`${s.name}: ${e instanceof Error ? e.message : "failed"}`); }
    }
    setBusy(false);
  ...
  async function rewriteWithAI(id: string, tone?: string) {
    setAiLoading(id);
    try {
      const r = await fnRewrite({ data: { itemId: id, tone } }) as { aiContent: string };
  ...
  async function toggleSource(s: ChannelSourceExt, v: boolean) {
  ...
  return (
    <>
      <PageHeader
        ...
        actions={
          <>
            <Btn variant="primary" onClick={pullAll} disabled={busy || sources.length === 0}>
              <RefreshCw className={`h-4 w-4 ${busy ? "animate-spin" : ""}`} /> Fetch New
            </Btn>
      ...
      <div className="grid lg:grid-cols-2 gap-4 mb-4">
              ...
              {sources.map((s) => (
                <div key={s.id} className="flex items-center justify-between gap-2 p-2 rounded bg-muted">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      onClick={async () => { if (confirm("Delete source?")) { await fnDel({ data: { id: s.id } }); invalidate(); } }}
        ...
        <Section title="How it works">
          <ol className="text-sm space-y-2 list-decimal pl-4">
            ...
            <li>Queue item par "Broadcast" — sab connected platforms par jayegi.</li>
          ...
          <div className="space-y-2">
            {queued.map((it) => (
              <div key={it.id} className="p-3 rounded bg-muted">
                <div className="text-xs text-muted-foreground mb-1">{it.channel_sources?.name} · {it.fetched_at ? new Date(it.fetched_at).toLocaleString() : ""}</div>
                {editing === it.id ? (
                  <div className="space-y-2">
                  ...
                  <>
                    ...
                    {it.ai_content && <div className="text-xs text-primary mt-1 flex items-center gap-1"><Sparkles className="h-3 w-3" /> AI rewritten</div>}
                    {(it.media_urls?.length ?? 0) > 0 && <div className="text-xs mt-1 text-muted-foreground">📎 {it.media_type} ({it.media_urls!.length})</div>}
                    <div className="flex gap-2 mt-2 flex-wrap">
                      ...
                      <Btn onClick={async () => { await fnDelItem({ data: { id: it.id } }); invalidateAll(); }}><Trash2 className="h-3 w-3" /></Btn>
                    </div>
                  </>
                )}
              </div>
                ...
                <tbody>
                  {done.slice(0, 20).map((it) => (
                    <tr key={it.id} className="border-t border-border">
                      ...
                      <td className="py-2"><Badge variant={it.status === "published" ? "success" : it.status === "failed" ? "warning" : "muted"}>{it.status}</Badge></td>
            ...
            <Card>
              ...
              <div className="space-y-3">
                ...
                <label className="block text-sm">Bot account
                  <select
                    value={form.bot_account_id}
                    onChange={(e) => setForm({ ...form, bot_account_id: e.target.value })}
