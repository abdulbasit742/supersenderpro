...
function CatalogPage() {
  ...
  useEffect(() => {
    if (!user) return;
    supabase.from("ecommerce_accounts").select("*").eq("user_id", user.id).eq("is_active", true).then(({ data }) => setEcomAccounts(data ?? []));
  }, [user]);

  const items = sampleProducts[active] ?? [];
  const selectedInCategory = selectedIds.filter(id => items.some((p: any) => p.id === id));
  ...
  return (
    <>
      <PageHeader title="Product Catalog" subtitle="AI tools, laptops, accessories aur food categories. Products ko marketplaces pe publish karein." actions={<Btn variant="primary">+ Add Product</Btn>} />
        ...
        {items.map((p) => (
          <Card key={p.id} className="flex flex-col relative">
            ...
            <div className="mt-2 flex flex-wrap gap-1">
              {p.tags.map((t: string) => <Badge key={t} variant="muted">{t}</Badge>)}
      ...
      {showPublish && (
        <PublishModal
          products={showPublish === "bulk"
            ? items.filter((p: any) => selectedIds.includes(p.id))
            : [sampleProducts[active].find((p:any) => p.id === showPublish)!].filter(Boolean)
          }
...
function PublishModal({ products, accounts, onClose }: { products: any[]; accounts: any[]; onClose: () => void }) {
    ...
    try {
      ...
      products.forEach(product => {
        selected.forEach(platform => {
          rows.push({ user_id: user!.id, product_id: product.id, platform, status: "draft" });
        });
      });
    ...
    <div className="fixed inset-0 z-50 bg-black/60 grid place-items-center p-4" onClick={onClose}>
      <div onClick={e => e.stopPropagation()} className="bg-card border border-border rounded-xl p-6 w-full max-w-md">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={publish} disabled={busy || selected.length === 0} className="flex-1 h-10 rounded-md bg-primary text-primary-foreground text-sm font-medium">{busy ? "Creating…" : `Create ${products.length * selected.length} Drafts`}</button>
        </div>
      </div>
    </div>
  );
...
function EditModal({ product, onClose }: { product: any; onClose: () => void }) {
