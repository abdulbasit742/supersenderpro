...
function buildSnapshot(storeId = 'default_store', now = Date.now()) { const customers = storeCRM ? (storeCRM.getAllCustomers(storeId) || []) : []; const r = engine.analyze(customers); return { storeId, generatedAt: new Date(now).toISOString(), summary: r.summary, lorenz: r.lorenz, topCustomers: r.topCustomers }; }
