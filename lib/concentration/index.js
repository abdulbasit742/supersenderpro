// lib/concentration/index.js
// Orchestrator over storeCRM. Reads customers, runs the concentration engine,
// masks the top-customer names' phones (names kept as-is since they're already
// on the profile and useful to the owner). Rebuilds nothing.
...
function buildSnapshot(storeId = 'default_store', now = Date.now()) {
  ...
  const r = engine.analyze(customers);
  return {
    storeId,
