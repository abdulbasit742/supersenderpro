// lib/messageScheduler/notify.js — Single outbound hook for scheduled sends. Draft-only until a
// notifier is wired AND live sends are enabled. Targets masked in returned payloads.
...
async function dispatch(target, message, opts = {}) {
 if (!config.effective.liveSends || !_notifier) return { sent: false, draft: true, to: maskContact(target), preview: message };
