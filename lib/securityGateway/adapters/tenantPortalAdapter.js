// lib/securityGateway/adapters/tenantPortalAdapter.js — Safe adapter for Tenant Portal.
...
function notify(summary) {
  if (!available()) return { module: 'Tenant Portal', available: false, delivered: false, reason: 'module_unavailable' };
  // Dry-run: record intent only. Never mutates source module, never calls external APIs.
  return { module: 'Tenant Portal', available: true, delivered: false, dryRun: true, payload: scrub(summary) };
}
module.exports = { name: 'Tenant Portal', available, notify };
