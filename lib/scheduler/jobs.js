...
function registerDefaults() {
  ...
  // Webhook auto-retry: re-send failed outbound deliveries (capped backoff, circuit-breaker guarded).
  scheduler.register('webhook-retry', {
    intervalMs: Number(process.env.WEBHOOK_RETRY_MS || 600000), // 10 min
    fn: async () => { let rj = null; try { rj = require('../webhooks/retryJob'); } catch { return; } await rj.runAll(); },
  });
}
