...
// - Real publishing is delegated to ./publishers (credential-gated, real HTTP
//   calls). With no platform token in env a job is marked 'skipped', never
//   faked as posted.
...
// --- real publishers --------------------------------------------------------
let publishers = {};
try { publishers = require('./publishers'); } catch (e) { publishers = {}; }
...
function jobPath(status, id) { return path.join(ROOT, status, `${id}.json`); }
...
async function generateContent({ topic, platforms, tone, mediaPath, mediaUrl, scheduledAt } = {}) {
...
// Attach/patch media (or other safe fields) onto a queued job and PERSIST it.
// Fixes a bug where media set in-memory was lost because scheduleJob re-reads
// the job from disk. read -> mutate -> write, all on the persisted record.
function attachMedia(id, { mediaPath, mediaUrl } = {}) {
  const job = readJob('queued', id);
  if (!job) throw new Error('queued job not found: ' + id);
  if (mediaPath !== undefined) job.mediaPath = mediaPath;
  if (mediaUrl !== undefined) job.mediaUrl = mediaUrl;
  job.updatedAt = new Date().toISOString();
  return writeJob('queued', job);
}
...
// A 'skipped' (no credentials) result is treated as failed so it stays visible.
...
// Publish a single queued job immediately by id (ignores schedule).
async function publishNow(id) {
  ...
  job.result = result;
  moveJob('queued', result.ok ? 'posted' : 'failed', job);
  return { id: job.id, platform: job.platform, result };
}
...
function status() {
  ...
  return {
    ...
    aiAvailable: !!(aiAgent && aiAgent.runPrompt),
    publishersLoaded: Object.keys(publishers),
  };
}
