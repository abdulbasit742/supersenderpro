...
//
// Design notes:
// - Pure CommonJS, no new dependencies (fs, path, crypto, https only).
...
// - Publishers (./publishers.js) are CREDENTIAL-GATED and make real HTTP calls
//   where implemented. With no platform token a job is marked 'skipped' with
//   the exact missing env var; nothing fakes a publish.
...
// --- real publishers --------------------------------------------------------
let platformPublishers = {};
try { platformPublishers = require('./publishers'); } catch (e) { platformPublishers = {}; }
...
// 'skipped' (missing creds) is treated as a non-fatal failure -> failed/ with reason.
async function publishDue() {
  const now = Date.now();
  const due = listJobs('queued').filter((j) => isDue(j, now));
  const results = [];
  for (const job of due) {
    const pub = platformPublishers[job.platform];
    let result;
    try {
...
// Publish a single queued job immediately (ignores schedule).
...
function status() {
  ...
  return {
    ...
    publishersLoaded: Object.keys(platformPublishers),
  };
}
