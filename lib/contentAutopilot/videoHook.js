...
function attachMediaToJob(orchestrator, jobId, fileName) {
  const jobs = orchestrator.listJobs('queued');
  ...
  if (base) job.mediaUrl = base.replace(/\/$/, '') + '/' + encodeURIComponent(fileName);
  // re-queue write via scheduleJob (keeps it in queued/, updates fields)
  return orchestrator.scheduleJob(jobId, job.scheduledAt || undefined);
}
