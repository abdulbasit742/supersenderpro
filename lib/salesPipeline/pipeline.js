...
// Fire-and-forget domain event (event bus #344). Lazy require avoids a load cycle; never blocks/throws.
function emitEvent(tid, event, payload) {
  try { const bus = require('../events/bus'); Promise.resolve(bus.emit(tid, event, payload)).catch(() => {}); } catch {}
}
...
function createDeal(tid, input = {}) {
  ...
  emitEvent(tid, 'deal.created', { dealId: deal.id, stage, value: deal.value });
...
function moveStage(tid, dealId, toStage, meta = {}) {
  ...
  // Domain events on terminal transitions (event bus #344 -> webhooks/audit/metrics)
  if (target.terminal) emitEvent(tid, target.outcome === 'won' ? 'deal.won' : 'deal.lost', { dealId, from, value: d.value, title: d.title });
  emitEvent(tid, 'deal.stage_changed', { dealId, from, to: target.id });
  return decorate(d);
}
