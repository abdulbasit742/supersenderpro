 ...
 * captures real payments (see Stripe path in lib/billing for live billing).
 */
...
// Fire-and-forget domain event (event bus #344). Lazy require; never blocks/throws.
...
function createQuote(tid, input = {}) {
  ...
  emitEvent(tid, 'quote.created', { number: doc.number, total: doc.total, dealId: doc.dealId });
  return doc;
}
...
function createInvoice(tid, input = {}) {
  ...
  emitEvent(tid, 'invoice.created', { number: doc.number, total: doc.total, dealId: doc.dealId });
  return doc;
}
...
function setStatus(tid, docId, status, extra = {}) {
  ...
  write(tid, data);
  if (status === 'paid' || status === 'paid_review') emitEvent(tid, 'invoice.paid', { number: doc.number, total: doc.total });
  return doc;
}
