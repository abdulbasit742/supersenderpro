// lib/analytics/notify.js — Single outbound hook for digest reports. Draft-only until a notifier
// is wired AND live digests are enabled. Recipient is masked in returned payloads.
...
async function dispatch(target, message, opts = {}) {
 if (!config.effective.liveDigests || !_notifier) return { sent: false, draft: true, to: _mask(target), preview: message };
