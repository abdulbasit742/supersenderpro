// lib/templateMarketplace/store.js — Safe atomic JSON store for the Template Marketplace.
// Demo/preview persistence only. Never stores secrets or raw customer/order/payment data.
...
function appendHistory(file, entry, max=200){ const list=readJSON(file, []); list.unshift({ ...entry, at:new Date().toISOString() }); writeJSON(file, list.slice(0,max)); return true; }
