...
const DEFAULT_SETTINGS = {
  ...
  competitorKeywords: [],
  blacklistSenders: [],
  groupChatCaptureDefault: true,
  directChatCaptureDefault: false,
  defaultTargetIds: [],
...
function createChannelAutomationCenter(opts = {}) {
  ...
  function normalizeSourceType(value) {
    ...
    if (['whatsapp_channel', 'channel', 'newsletter'].includes(type)) return 'whatsapp_channel';
    if (['whatsapp_group', 'group', 'group_chat'].includes(type)) return 'whatsapp_group';
    if (['whatsapp_chat', 'chat', 'direct', 'dm'].includes(type)) return 'whatsapp_chat';
    if (['webhook', 'website', 'rss', 'api', 'ecommerce'].includes(type)) return type;
  ...
  function addSource(input = {}) {
    ...
    const rec = {
      ...
      link: input.link || '',
      sourceType: normalizeSourceType(input.sourceType || input.type || existingSourceType(input)),
  ...
  function existingSourceType(input = {}) {
    const raw = String(input.channelId || input.link || input.id || '').toLowerCase();
    if (raw.endsWith('@g.us') || raw.includes('g.us')) return 'whatsapp_group';
    if (raw.endsWith('@s.whatsapp.net') || raw.includes('@c.us')) return 'whatsapp_chat';
    if (raw.includes('@newsletter') || raw.includes('whatsapp.com/channel')) return 'whatsapp_channel';
    return 'whatsapp_channel';
  }
  ...
  function addTarget(input = {}) {
    ...
    const rec = {
      ...
      channelId: input.channelId || '',
      platform: input.platform || 'whatsapp', // whatsapp|whatsapp_group|telegram|facebook|instagram|linkedin|tiktok
  ...
  function resolveTargetsForSource(src) {
    ...
    const ids = (src?.rules?.targets && src.rules.targets.length)
      ? src.rules.targets
      : (Array.isArray(settings.defaultTargetIds) ? settings.defaultTargetIds : []);
  ...
  function normalizeIncomingContent(input = {}, sourceType = 'webhook') {
    const text = extractMessageText(input);
    const channelId = input.channelId || input.groupId || input.chatId || input.from || input.sourceId || '';
    const mediaUrl = input.mediaUrl || input.imageUrl || input.videoUrl || input.documentUrl || '';
    return {
      sourceId: input.sourceId || '',
      channelId,
      sourceType: normalizeSourceType(sourceType || input.sourceType || input.type),
      text,
      mediaUrl,
      mediaType: input.mediaType || (input.videoUrl ? 'video' : input.imageUrl ? 'image' : mediaUrl ? 'image' : 'text'),
      mediaBytes: Number(input.mediaBytes || 0),
      sender: input.sender || input.author || input.senderNumber || '',
  ...
  function findSourceForContent(content = {}) {
    ...
    return sources.find(s =>
      s.id === content.sourceId ||
      s.channelId === content.channelId ||
      s.link === content.channelId ||
      (s.link && content.link && s.link === content.link)
    );
  }

  async function ingestGroupChatMessage(input = {}, options = {}) {
    const content = normalizeIncomingContent(input, 'whatsapp_group');
    const settings = getSettings();
    if (settings.groupChatCaptureDefault === false && !options.force) {
      return { accepted: false, queued: false, steps: ['group-capture-disabled'], dryRun: settings.dryRun };
    }
    if (!content.channelId || !content.text) {
      return { accepted: false, queued: false, steps: ['missing-group-or-text'], dryRun: settings.dryRun };
    }
    return ingestSourcePost(content, options);
  }

  async function ingestDirectChatMessage(input = {}, options = {}) {
    const content = normalizeIncomingContent(input, 'whatsapp_chat');
    const settings = getSettings();
    if (settings.directChatCaptureDefault === false && !options.force) {
      return { accepted: false, queued: false, steps: ['direct-chat-capture-disabled'], dryRun: settings.dryRun };
    }
    if (!content.channelId || !content.text) {
      return { accepted: false, queued: false, steps: ['missing-chat-or-text'], dryRun: settings.dryRun };
    }
    return ingestSourcePost(content, options);
  }

  function previewContentFlow(input = {}) {
    const settings = getSettings();
    const content = normalizeIncomingContent(input, input.sourceType || input.type || 'webhook');
    const source = findSourceForContent(content);
    const result = {
      accepted: false,
      dryRun: true,
      liveSend: false,
      sourceFound: !!source,
      source: source ? { id: source.id, name: source.name, sourceType: source.sourceType, category: source.category } : null,
      targets: [],
      warnings: [],
      blockers: []
    };
    if (!content.text && !content.mediaUrl) result.blockers.push('message text or media is required');
    if (!source) result.blockers.push('source is not registered');
    if (source && source.listType === 'blacklist') result.blockers.push('source is blacklisted');
    if (source) {
      const safety = runSafetyChecks(content, source, settings);
      if (!safety.ok) result.blockers.push(...safety.reasons);
      const targets = resolveTargetsForSource(source);
      result.targets = targets.map(t => ({
        id: t.id,
        name: t.name,
        platform: t.platform,
        previewText: applyBranding(cleanCaption(content.text, source, settings), t, source, settings)
      }));
      if (!targets.length) result.warnings.push('no target mapped for this source');
    }
    result.accepted = result.blockers.length === 0;
    return result;
  }

  function contentFlowStatus() {
    const sources = refreshSourceHealth();
    const targets = getTargets();
    const queue = getQueue();
    const logs = getLogs();
    const byType = sources.reduce((acc, item) => {
      const key = normalizeSourceType(item.sourceType);
      acc[key] = (acc[key] || 0) + 1;
      return acc;
    }, {});
    const byPlatform = targets.reduce((acc, item) => {
      const key = String(item.platform || 'whatsapp').toLowerCase();
      acc[key] = (acc[key] || 0) + 1;
      return acc;
    }, {});
    return {
      ...
      queue: queue.length,
      pendingApproval: queue.filter(row => row.status === 'pending_approval').length,
  ...
  async function ingestSourcePost(post = {}, options = {}) {
    ...
    // locate source
    const sources = getSources();
    let src = sources.find(s => s.id === post.sourceId || s.channelId === post.channelId || s.link === post.channelId || (s.link && post.link && s.link === post.link));
  ...
  async function handleAdminCommand(command, args = []) {
    ...
    switch (cmd) {
      ...
      case '!contentflow':
      case '!flowstatus': {
        const s = contentFlowStatus();
        const sourceLines = Object.entries(s.byType).map(([k, v]) => `${k}: ${v}`).join(', ') || 'none';
        const targetLines = Object.entries(s.byPlatform).map(([k, v]) => `${k}: ${v}`).join(', ') || 'none';
        return [
          '*Content Flow OS*',
          `Mode: ${s.dryRun ? 'DRY-RUN' : 'LIVE'}${s.paused ? ' / PAUSED' : ''}`,
          `Sources: ${s.sources} (${sourceLines})`,
          `Targets: ${s.targets} (${targetLines})`,
          `Queue: ${s.queue} | Approval: ${s.pendingApproval} | Approved: ${s.approved}`,
          'Commands: !channelsources, !channeltargets, !channelqueue'
        ].join('\n');
      }
      case '!channelsources': {
  ...
  const ADMIN_COMMANDS = ['!channelstatus', '!contentflow', '!flowstatus', '!channelsources', '!channeltargets', '!channelqueue', '!channelapprove', '!channelreject', '!channelpublish', '!pausechannels', '!resumechannels', '!addsource', '!addtarget', '!route', '!digest', '!channeldoctor'];
  ...
  return {
    ...
    // pipeline / queue
    ingestSourcePost, ingestGroupChatMessage, ingestDirectChatMessage, previewContentFlow, contentFlowStatus,
    getQueue, findQueueItem, approveQueueItem, rejectQueueItem, publishQueueItem, processQueueTick,
    // ecommerce
    ecommerceEvent,
    // settings / control
