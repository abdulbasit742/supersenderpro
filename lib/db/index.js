...
function postgresDriver() {
  ...
  // collection (lib/db) -> Prisma delegate name. Keep in sync with prisma/schema.prisma @@map tables.
  const MODEL = {
    customers: 'customer',
    orders: 'order',
    quotes: 'quote',
    ...
    follow_ups: 'followUp',
    // added subsystems (PR #152 schema):
    users: 'user',
  ...
  const delegate = (collection) => {
    ...
    const d = client()[name];
    if (!d) throw new Error('Unknown collection/model: ' + collection + ' (add it to lib/db MODEL map + prisma schema)');
    return d;
  };
  return {
    driver: 'postgres',
