...
// ── New analytics features (read-only over current graph) ──
const trending = (hours) => features.trendingSkus(store.read(), Number(hours) || 24);
const heatmap = () => features.cityHeatmap(store.read());
const categoryBreakdown = () => features.categories(store.read());
const priceHistory = (sku) => features.priceHistory(store.read(), sku);
const forecast = (sku) => features.demandForecast(store.read(), sku);
const competitorIndex = () => features.competitorPriceIndex(store.read());
const bestTimeToBuy = (sku) => features.bestTimeToBuy(store.read(), sku);
const negotiation = (sku) => features.negotiation(store.read(), sku);
const bundles = () => features.bundleOpportunities(store.read());
const stockoutRisk = () => features.stockoutRisk(store.read());
const duplicates = () => features.duplicateOffers(store.read());
const supplyGap = () => features.demandSupplyGap(store.read());
const anomalies = () => features.priceAnomalies(store.read());
const dropLeaders = () => features.priceDropLeaders(store.read());
const summary = () => features.marketSummary(store.read());
...
module.exports = {
  ...
  // analytics
  trending, heatmap, categoryBreakdown, priceHistory, forecast, competitorIndex,
  bestTimeToBuy, negotiation, bundles, stockoutRisk, duplicates, supplyGap,
  anomalies, dropLeaders, summary,
  // stateful lists
  lists, features,
