...
/**
 * Ecommerce Hub — WhatsApp command handlers (customer-facing) + order replies.
 ...
 *   1. !track <orderId>  (buyer order tracking)
 *   2. !shop / !product <id> / !orders | !clients  (customer browse)
 *
 * Commands:
 *   !shop                -> list products across all platforms (paged)
 *   !product <id>        -> product details + link
 *   !orders | !clients   -> masked client/order summary
 *   !track [orderId]     -> tracking status + link
 */
 ...
 * fromNumber is optional but required to match COD confirmations + latest track.
 */
