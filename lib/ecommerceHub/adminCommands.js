...
function help() {
  return [
    ...
    '',
    'Customers ke liye: !shop, !product <id>, !orders'
  ].join('\n');
}
...
/**
 * handle(text, fromNumber) -> reply string, or null if not an admin command.
 * Returns an auth message if a write command is used by a non-admin.
 */
