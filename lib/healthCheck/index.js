...
// Readiness: can we actually serve? (data dir + json store must be ok)
async function getReadiness() {
  const [dir, store] = await Promise.all([runProbe('dataDir', dataDirWritable), runProbe('jsonStore', jsonStoreReadable)]);
  const status = (dir.status === 'down') ? 'down' : rollup({ dir, store });
  return { status, checks: { dataDir: dir, jsonStore: store } };
}
