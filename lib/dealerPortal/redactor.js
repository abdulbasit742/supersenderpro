...
function maskOrderRef() { return 'ord_****'; }
function maskInvoiceRef() { return 'inv_****'; }
function maskShipmentRef() { return 'ship_****'; }
...
function redactShipment(record = {}) {
  return {
    shipmentIdPreview: maskRef(record.id || 'ship', 'ship'),
    trackingRefMasked: maskShipmentRef(record.trackingRef),
    statusPreview: `${safeText(record.status || 'pending')}_preview`,
  };
}
...
module.exports = {
  ...
  maskPaymentRef, maskCreditRef, maskTaxRef, maskDocumentRef, maskCustomerRef, maskOrderRef, maskInvoiceRef, maskShipmentRef, safeText,
  redactDealer, redactCustomer, redactPayment, redactCredit, redactTax, redactDocument, redactOrder, redactInvoice, redactShipment,
  hasLeak,
};
