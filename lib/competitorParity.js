...
/**
 * ========================================================================
 * NEW FEATURE BATCH 5 (A/B Testing, Send-Time, Merge Fields, Compliance, Stock Forecast, Health Score)
 * ========================================================================
 */

const ABTEST_FILE = path.join(DATA_DIR, 'ab_variants.json');

/**
 * 1. A/B TEST VARIANT ENGINE
 * Creates a message A/B test and picks a winning variant by conversion rate.
 */
function createAbTest(tenantId, name, variants = []) {
  const test = {
    id: `ABT-${Date.now()}`,
    tenantId, name,
    variants: variants.map((v, i) => ({ id: `V${i + 1}`, text: String(v), sent: 0, conversions: 0 })),
    status: 'running', createdAt: new Date().toISOString()
  };
  let db = [];
  try { if (fs.existsSync(ABTEST_FILE)) db = JSON.parse(fs.readFileSync(ABTEST_FILE, 'utf8')) || []; } catch {}
  db.push(test);
  fs.writeFileSync(ABTEST_FILE, JSON.stringify(db, null, 2));
  return test;
}

function recordAbResult(tenantId, testId, variantId, converted = false) {
  let db = [];
  try { if (fs.existsSync(ABTEST_FILE)) db = JSON.parse(fs.readFileSync(ABTEST_FILE, 'utf8')) || []; } catch {}
  const test = db.find(t => t.tenantId === tenantId && t.id === testId);
  if (!test) return { success: false, error: 'Test not found.' };
  const variant = test.variants.find(v => v.id === variantId);
  if (!variant) return { success: false, error: 'Variant not found.' };
  variant.sent += 1;
  if (converted) variant.conversions += 1;

  const ranked = test.variants.map(v => ({ ...v, rate: v.sent ? v.conversions / v.sent : 0 })).sort((a, b) => b.rate - a.rate);
  test.winner = ranked[0].sent >= 10 ? ranked[0].id : null;
  fs.writeFileSync(ABTEST_FILE, JSON.stringify(db, null, 2));
  return { success: true, leaderboard: ranked, winner: test.winner };
}

/**
 * 2. BEST-TIME-TO-SEND RECOMMENDER
 * Suggests optimal broadcast windows based on segment activity heuristics.
 */
function recommendSendTime(segment = 'general', timezone = 'Asia/Karachi') {
  const windows = {
    general: ['12:30', '20:00'],
    students: ['16:00', '21:30'],
    business: ['10:00', '15:00'],
    resellers: ['11:00', '22:00']
  };
  const slots = windows[String(segment).toLowerCase()] || windows.general;
  return {
    segment, timezone,
    recommendedSlots: slots,
    note: 'Avoid 1am-7am. Best engagement is lunch and post-dinner windows in PK.',
    nextBestSlot: slots[0]
  };
}

/**
 * 3. MESSAGE PERSONALIZATION MERGE-FIELD RENDERER
 * Replaces {{field}} tokens with contact data, with safe fallbacks.
 */
function renderMergeMessage(template, data = {}) {
  const missing = [];
  const rendered = String(template || '').replace(/\{\{\s*([a-zA-Z0-9_]+)\s*\}\}/g, (m, key) => {
    if (data[key] !== undefined && data[key] !== null && data[key] !== '') return String(data[key]);
    missing.push(key);
    return '';
  });
  return { rendered: rendered.replace(/\s{2,}/g, ' ').trim(), missingFields: missing, complete: missing.length === 0 };
}

/**
 * 4. COMPLIANCE & SPAM PRE-SEND CHECKER
 * Flags risky content before a broadcast goes out.
 */
function checkMessageCompliance(message) {
  const text = String(message || '');
  const lower = text.toLowerCase();
  const issues = [];
  const spamWords = ['100% free', 'guaranteed', 'winner', 'click here now', 'limited time only', 'act now', 'congratulations you won'];
  spamWords.forEach(w => { if (lower.includes(w)) issues.push(`Spam trigger phrase: "${w}"`); });
  const caps = (text.match(/[A-Z]/g) || []).length;
  if (text.length > 20 && caps / text.length > 0.5) issues.push('Excessive capital letters.');
  if ((text.match(/[!]{2,}/g) || []).length) issues.push('Multiple exclamation marks.');
  if ((text.match(/https?:\/\//g) || []).length > 2) issues.push('Too many links (>2).');
  if (text.length > 1000) issues.push('Message too long (>1000 chars).');
  ...
  return {
    risk, issues,
    safeToSend: risk !== 'high',
    recommendation: risk === 'high' ? 'Revise the message before sending to avoid bans.' : (risk === 'medium' ? 'Minor cleanup recommended.' : 'Looks clean. Safe to send.')
  };
}

/**
 * 5. STOCK DEMAND FORECASTER
 * Predicts days-until-stockout and reorder suggestion from sales velocity.
 */
function forecastStockDemand(productId, currentStock, dailySalesAvg, leadTimeDays = 3) {
  const velocity = Math.max(0.01, Number(dailySalesAvg || 0));
  const daysLeft = Math.floor(Number(currentStock || 0) / velocity);
  const reorderNow = daysLeft <= leadTimeDays;
  const suggestedReorderQty = Math.ceil(velocity * (leadTimeDays + 14)); // 2-week buffer

  return {
    productId,
    currentStock: Number(currentStock || 0),
    dailySalesAvg: velocity,
    daysUntilStockout: daysLeft,
    reorderNow,
    suggestedReorderQty,
    status: reorderNow ? `⚠️ Reorder now! Only ~${daysLeft} days of stock left.` : `✅ Stock healthy for ~${daysLeft} days.`
  };
}

/**
 * 6. CUSTOMER HEALTH SCORE
 * Combines recency, frequency, monetary, and satisfaction into a single 0-100 health score.
 */
function computeCustomerHealth(options = {}) {
  const recencyDays = Number(options.recencyDays || 30);
  const totalOrders = Number(options.totalOrders || 1);
  const totalSpent = Number(options.totalSpent || 0);
  const npsScore = Number(options.npsScore || 7);

  let score = 50;
  score += recencyDays <= 7 ? 20 : (recencyDays <= 30 ? 10 : (recencyDays <= 90 ? 0 : -20));
  score += Math.min(20, totalOrders * 3);
  score += totalSpent > 20000 ? 15 : (totalSpent > 5000 ? 8 : 2);
  score += (npsScore - 7) * 3;
  ...
  return {
    ...
    factors: { recencyDays, totalOrders, totalSpent, npsScore },
    action: status === 'At Risk' ? 'Trigger win-back + personal outreach.' : (status === 'Stable' ? 'Nurture with loyalty offers.' : 'Upsell and request referrals.')
  };
}
...
/**
 * ========================================================================
 * NEW FEATURE BATCH 6 (Smart Auto-Reply Router, Upsell Bundle Builder, Payment Reminder Scheduler)
 * ========================================================================
 */

const BUNDLE_FILE = path.join(DATA_DIR, 'upsell_bundles.json');
const PAYMENT_REMINDER_FILE = path.join(DATA_DIR, 'payment_reminders.json');

/**
 * 1. SMART AUTO-REPLY INTENT ROUTER
 * Classifies an incoming message into an intent and returns a suggested auto-reply.
 */
function smartAutoReply(message) {
  const text = String(message || '').toLowerCase();
  const intents = [
    { intent: 'pricing', keywords: ['price', 'rate', 'kitna', 'kitne', 'cost', 'paisa', 'kimat'], reply: 'Our prices are very competitive! Please tell me which product/tool you are interested in and I will share the exact rate.' },
    { intent: 'availability', keywords: ['stock', 'available', 'mojud', 'hai kya', 'milega'], reply: 'Let me check availability for you. Which item would you like?' },
    { intent: 'order', keywords: ['order', 'buy', 'kharidna', 'lena hai', 'chahiye'], reply: 'Great! I can place your order right now. Please confirm the product and quantity.' },
    { intent: 'payment', keywords: ['payment', 'jazzcash', 'easypaisa', 'bank', 'transfer', 'paid'], reply: 'You can pay via JazzCash, EasyPaisa, or Bank transfer. Share the screenshot after payment and we will verify instantly.' },
    { intent: 'support', keywords: ['problem', 'issue', 'help', 'masla', 'kaam nahi', 'not working'], reply: 'Sorry for the trouble! Please describe the issue and our support team will resolve it quickly.' },
    { intent: 'greeting', keywords: ['hi', 'hello', 'salam', 'assalam', 'aoa'], reply: 'Walaikum Assalam! Welcome 😊 How can I help you today?' }
  ];

  let best = { intent: 'unknown', score: 0, reply: 'Thanks for your message! A team member will assist you shortly.' };
  for (const it of intents) {
    let score = 0;
    for (const k of it.keywords) if (text.includes(k)) score += 1;
    if (score > best.score) best = { intent: it.intent, score, reply: it.reply };
  }
  return { matchedIntent: best.intent, confidence: best.score, suggestedReply: best.reply };
}

/**
 * 2. UPSELL BUNDLE BUILDER
 * Creates a discounted bundle from multiple products and computes savings.
 */
function buildUpsellBundle(tenantId, bundleName, items = [], bundleDiscountPercent = 15) {
  const subtotal = items.reduce((sum, it) => sum + (Number(it.price || 0) * Number(it.qty || 1)), 0);
  const discount = subtotal * (Number(bundleDiscountPercent) / 100);
  const finalPrice = subtotal - discount;

  const bundle = {
    id: `BUNDLE-${Date.now()}`,
    tenantId, bundleName, items,
    subtotal: parseFloat(subtotal.toFixed(2)),
    bundleDiscountPercent: Number(bundleDiscountPercent),
    savings: parseFloat(discount.toFixed(2)),
    finalPrice: parseFloat(finalPrice.toFixed(2)),
    pitch: `🎁 ${bundleName}: Get all ${items.length} items for Rs ${finalPrice.toFixed(0)} (save Rs ${discount.toFixed(0)}, ${bundleDiscountPercent}% off)!`,
    createdAt: new Date().toISOString()
  };
  let db = [];
  try { if (fs.existsSync(BUNDLE_FILE)) db = JSON.parse(fs.readFileSync(BUNDLE_FILE, 'utf8')) || []; } catch {}
  db.push(bundle);
  fs.writeFileSync(BUNDLE_FILE, JSON.stringify(db, null, 2));
  return bundle;
}

/**
 * 3. PAYMENT REMINDER SCHEDULER
 * Schedules escalating reminders for a pending/unpaid order.
 */
function schedulePaymentReminder(tenantId, phone, orderRef, amount, dueInHours = 24) {
  const now = Date.now();
  const reminder = {
    id: `PAYREM-${Date.now()}`,
    tenantId, phone, orderRef, amount: Number(amount || 0),
    dueAt: new Date(now + (Number(dueInHours) * 3600000)).toISOString(),
    schedule: [
      { at: new Date(now + 3600000).toISOString(), text: `Hi! Friendly reminder: your order ${orderRef} (Rs ${amount}) is awaiting payment.` },
      { at: new Date(now + (Number(dueInHours) / 2 * 3600000)).toISOString(), text: `Reminder: please complete payment for order ${orderRef} to avoid cancellation.` },
      { at: new Date(now + (Number(dueInHours) * 3600000)).toISOString(), text: `Final notice: order ${orderRef} will be cancelled soon if payment (Rs ${amount}) is not received.` }
    ],
    status: 'scheduled', createdAt: new Date().toISOString()
  };
  let db = [];
  try { if (fs.existsSync(PAYMENT_REMINDER_FILE)) db = JSON.parse(fs.readFileSync(PAYMENT_REMINDER_FILE, 'utf8')) || []; } catch {}
  db.push(reminder);
  fs.writeFileSync(PAYMENT_REMINDER_FILE, JSON.stringify(db, null, 2));
  return reminder;
}
