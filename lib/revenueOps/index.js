...
const { aging } = require('./dealAging');
const { salesVelocity } = require('./salesVelocity');
const { quotaAttainment } = require('./quotaAttainment');
const { winLoss } = require('./winLossAnalysis');
const { nextBestActions } = require('./nextBestAction');
const { funnel } = require('./funnelAnalysis');
const { exportPreview } = require('./exportPreview');
...
function getRevenueOpsStatus() {
  return S({
    ...
    supportedModules: ['pipeline', 'leads', 'opportunities', 'deal-score', 'forecast', 'followup',
      ...
      'lead-score', 'deal-aging', 'sales-velocity', 'quota-attainment', 'win-loss', 'next-best-actions',
      'funnel', 'export-preview'],
    warnings: [], blockers: [],
  });
}
...
function calculateLeadScorePreview(input) {
  const lead = input && (input.lead || input);
  return S(scoreLead(lead || {}));
}
function getDealAgingPreview(input) {
  const deals = (input && Array.isArray(input.opportunities) && input.opportunities.length) ? input.opportunities : getSampleOpportunities();
  return S(aging(deals));
}
function calculateSalesVelocityPreview(input) {
  const deals = (input && Array.isArray(input.opportunities) && input.opportunities.length) ? input.opportunities : getSampleOpportunities();
  return S(salesVelocity(deals));
}
function calculateQuotaAttainmentPreview(input) {
  const deals = (input && Array.isArray(input.opportunities) && input.opportunities.length) ? input.opportunities : getSampleOpportunities();
  return S(quotaAttainment(input || {}, deals));
}
function getWinLossAnalysisPreview(input) {
  const deals = (input && Array.isArray(input.opportunities) && input.opportunities.length) ? input.opportunities : getSampleOpportunities();
  return S(winLoss(deals));
}
function getNextBestActionsPreview(input) {
  const deals = (input && Array.isArray(input.opportunities) && input.opportunities.length) ? input.opportunities : getSampleOpportunities();
  return S({ nextBestActionsPreview: nextBestActions(deals) });
}
function getFunnelAnalysisPreview(input) {
  const deals = (input && Array.isArray(input.opportunities) && input.opportunities.length) ? input.opportunities : getSampleOpportunities();
  return S(funnel(deals));
}
function getExportPreview(input) {
  const deals = (input && Array.isArray(input.opportunities) && input.opportunities.length) ? input.opportunities : getSampleOpportunities();
  return S(exportPreview(deals));
}
...
module.exports = {
  ...
  // extended analytics
  calculateLeadScorePreview,
  getDealAgingPreview,
  calculateSalesVelocityPreview,
  calculateQuotaAttainmentPreview,
  getWinLossAnalysisPreview,
  getNextBestActionsPreview,
  getFunnelAnalysisPreview,
  getExportPreview,
};
