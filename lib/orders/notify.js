// lib/orders/notify.js — Single outbound hook for order status messages to the customer. Draft-only
// until a notifier is wired AND live messages are enabled. Respects consent (#38) when present.
...
async function dispatch(target, message, opts = {}) {
 ...
 if (!config.liveMessages || !_notifier) return { sent: false, draft: true, to: maskContact(target), preview: message };
