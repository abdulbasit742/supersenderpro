'use strict';
/**
 * lib/bootstrap/registerSubsystems.js - one entry point that wires every subsystem into an
 * existing Express app, in the correct order, each guarded so a single failure can't abort boot.
 *
 ...
 *   0b CORS
 *   0c maintenance guard (503 when in maintenance; allowlist passes)
 *   0d request timeout + slow-request log
 *   1  observability request tracing + http metrics
 *   2  rate-limit guards
 *   3  feature routers (all subsystems)
 *   4  404 (api) + error handler (last)
 *   5  background: uptime, scheduler, admin-alert polling, graceful shutdown
 */
...
function registerAll(app, server) {
  ...
  // 0c) maintenance guard (after CORS so preflight still works; allowlist keeps health/version/metrics up)
  safe(report, 'maintenance.guard', () => { const m = require('../maintenance/middleware'); app.use(m.maintenanceGuard()); });
  // 0d) request timeout + slow log
  ...
  const routers = [
    ...
    // newer subsystems (#336-#339)
    ['webhookLog', '/api/webhooks', '../../routes/webhookLogRoutes'],
    ['analytics', '/api/analytics', '../../routes/analyticsRoutes'],
    ['customersCsv', '/api/customers', '../../routes/customerCsvRoutes'],
    ['maintenance', '/api/maintenance', '../../routes/maintenanceRoutes'],
  ];
  ...
  safe(report, 'maintenance.load', () => { require('../maintenance').loadPersisted(); });
