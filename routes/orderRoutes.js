...
//
// Wire-up (server.js) — on order, record to 360 + award loyalty + create invoice:
//   const orders = require('./lib/commerce/orders');
//   orders.setOnOrder((o) => {
//     require('./lib/crm/customer360').recordEvent(o.phone, { type:'order', amount:o.total, ref:o.id });
//     require('./lib/marketing/loyaltyEngine').earnFromOrder({ phone:o.phone }, o.total, `order ${o.id}`);
//   });
...
router.post('/cart/:phone/remove', (req, res) => {
  if (!ensure(res)) return;
  const c = orders.removeFromCart(req.params.phone, (req.body || {}).productId);
  if (!c) return res.status(404).json({ ok: false, error: 'No cart' });
